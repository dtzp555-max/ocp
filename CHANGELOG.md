...
- **OCP's own inbound credentials no longer reach a spawned `claude` (#328).** `spawnClaudeProcess` built the child environment as `{...process.env}` and stripped only four `ANTHROPIC_*` variables, so `PROXY_API_KEY` — the credential that authenticates callers **to** OCP — was handed to every child. The child is the one component that reads attacker-controlled text, and it has no use for that key: it authenticates to Anthropic with `CLAUDE_CODE_OAUTH_TOKEN` and never talks to OCP. `OCP_ADMIN_KEY` and `PROXY_ANONYMOUS_KEY` leaked the same way.
  ...
  - **A frozen list, not four more `delete` lines.** `INBOUND_AUTH_ENV_VARS` + `scrubInboundAuthEnv()` in `lib/spawn-auth.mjs` are what both scrub sites call — the request spawn and the background auth probe. Those two sites had already drifted in purpose, and a denylist you must remember to extend in both places is precisely the failure mode that produced this. A new inbound-auth variable is added in one place.
  - Deliberately **not** stripped: `CLAUDE_CODE_OAUTH_TOKEN` (an outbound credential the child genuinely needs — a unit test pins that it survives, because stripping it would break every request) and `PROXY_ADVERTISE_ANON_KEY` (a boolean flag, not a secret).
