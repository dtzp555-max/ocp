...
- **OCP's own inbound credentials no longer reach a spawned `claude`, on every spawn path (#328).** `spawnClaudeProcess` built the child environment as `{...process.env}` and stripped only four `ANTHROPIC_*` variables, so `PROXY_API_KEY` — the credential that authenticates callers **to** OCP — was handed to every child. The child is the one component that reads attacker-controlled text, and it has no use for that key: it authenticates to Anthropic with `CLAUDE_CODE_OAUTH_TOKEN` and never talks to OCP. `OCP_ADMIN_KEY` and `PROXY_ANONYMOUS_KEY` leaked the same way.
  ...
  - **There were FOUR sites, not two, and two of them were found only by independent review.** The third was the one that mattered most. An independent review found `lib/tui/session.mjs` carrying its own copy of the same four-name denylist, so every secret reached the tmux pane. That is the **only** request path when `CLAUDE_TUI_MODE=true`: `server.mjs` selects `callClaudeTui` over `callClaude` wholesale, so a fix confined to `spawnClaudeProcess` would have left every TUI request unprotected while the changelog claimed otherwise. Both the pane's `env -u` prefix and the tmux **server's** own environment are scrubbed now — and the second is not merely belt-and-braces: a reviewer measured that with only the pane prefix in place, `tmux show-environment -g` **still returned all three secrets from inside the pane**, which is a cross-platform retrieval path, not just the Linux `/proc` one — a server started by OCP inherits OCP's environment and its panes inherit from the server, and OCP's boot reap calls `tmux kill-server`, which guarantees the next `new-session` rebuilds it under that environment. So the leak was the default path, not an edge case.
  ...
  - Deliberately **not** stripped: `CLAUDE_CODE_OAUTH_TOKEN` (an outbound credential the child genuinely needs — an integration assertion pins that it reaches the child, and the isolated-spawn branch re-injects it downstream of the scrub anyway, so no edit at the scrub site can break requests) and `PROXY_ADVERTISE_ANON_KEY` (a boolean flag, not a secret).
  ...
  - **Six tests, all behavioural.** The fake `claude` dumps its own environment and the assertions read it; nothing greps source. Both scrub sites are covered **separately** — removing either one turns exactly one integration test red while the other stays green, so a half-fix cannot pass. Each test also asserts a control (the capture is non-empty and looks like an environment) so a broken capture cannot satisfy "no secret found" vacuously.
