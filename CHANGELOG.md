...
- **The auth probe read different credentials than the request path, so `/health`'s `auth` verdict predicted nothing (#308).** `checkAuth`'s own comment claimed it "resolves the SAME credentials the request path uses". Scrubbing `ANTHROPIC_*` identically was necessary but not sufficient: the request path also applies a per-spawn HOME decision (`getSpawnHomeMode`) and the probe did not. With `CLAUDE_CODE_OAUTH_TOKEN` set, spawns run under a credential-free scratch HOME where the env token is the only credential, while the probe ran under the operator's real HOME — where `claude` **prefers** a `.credentials.json` over the env var (the same preference ADR 0007 PR-D documents for the TUI path). Two credential sources, and the verdict was wrong in both directions: a valid `credentials.json` with a dead env token reports `authenticated` while every request 500s (#308's reported shape), and the reverse reports `rejected` while serving is fine (the #232 false-alarm class, from the other end).
  - **Confirmed live on a production host mid-triage**, not inferred: request path `spawn.home` `/home/opc/.ocp/spawn-home`, probe `HOME` `/home/opc`, and a 108-character `accessToken` sitting in the latter's `.credentials.json`. The originally reported host had already been re-credentialed, so the repro was gone — this is the same defect, found by reading and then verified on a different machine.
  - `getSpawnHomeMode()` is used rather than `resolveSpawnDecision()`: the config-level decision is synchronous, has no fs side effects, and takes no lock. The probe must **not** take `realHomeFallbackMutex` — that lock serializes real-HOME spawns to protect the single-use refresh token, and a periodic health probe queueing behind request traffic would add latency for nothing.
  - Deliberately left open, and documented in the code: inside the 5-minute token-expiry window the request path falls back to the real HOME to let `claude` refresh natively, while the probe stays isolated. That window is bounded and self-clearing; closing it would mean taking the mutex.
