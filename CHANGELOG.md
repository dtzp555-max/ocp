...
- **OCP's own inbound credentials no longer reach a spawned `claude`, on all three spawn paths (#328).** `spawnClaudeProcess` built the child environment as `{...process.env}` and stripped only four `ANTHROPIC_*` variables, so `PROXY_API_KEY` — the credential that authenticates callers **to** OCP — was handed to every child. The child is the one component that reads attacker-controlled text, and it has no use for that key: it authenticates to Anthropic with `CLAUDE_CODE_OAUTH_TOKEN` and never talks to OCP. `OCP_ADMIN_KEY` and `PROXY_ANONYMOUS_KEY` leaked the same way.
  ...
  - **There were THREE sites, not two — and the third was the one that mattered most.** An independent review found `lib/tui/session.mjs` carrying its own copy of the same four-name denylist, so every secret reached the tmux pane. That is the **only** request path when `CLAUDE_TUI_MODE=true`: `server.mjs` selects `callClaudeTui` over `callClaude` wholesale, so a fix confined to `spawnClaudeProcess` would have left every TUI request unprotected while the changelog claimed otherwise. Both the pane's `env -u` prefix and the tmux **server's** own environment are scrubbed now — a server started by OCP inherits OCP's environment and its panes inherit from the server, and OCP's boot reap calls `tmux kill-server`, which guarantees the next `new-session` rebuilds it under that environment. So the leak was the default path, not an edge case.
  - **A frozen list, not four more `delete` lines.** `INBOUND_AUTH_ENV_VARS` + `scrubInboundAuthEnv()` in `lib/spawn-auth.mjs` are what all three sites call — the request spawn, the background auth probe, and the TUI pane. Those two sites had already drifted in purpose, and a denylist you must remember to extend in both places is precisely the failure mode that produced this. A new inbound-auth variable is added in one place.
