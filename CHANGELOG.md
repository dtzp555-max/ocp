...
- **`/health` reported `auth.ok: true` on hosts where no request could be served (#308, ADR 0014 — Proposed, needs maintainer sign-off).** Root cause, measured rather than reasoned about: **`claude auth status` reports whether a token is PRESENT, not whether it is VALID.** In an isolated empty `HOME`, with no token it exits 1 (`loggedIn: false`); with a **fabricated** token — a string that never existed — it exits **0** and reports `loggedIn: true, authMethod: "oauth_token"`.
  ...
  - **A completed request is now conclusive evidence**, and free: it was happening anyway. A successful completion sets `auth.ok: true` / `lastOutcome: "verified-by-request"`.
  ...
  - **`/health`'s `status` is untouched.** `proxyHealthStatus` reads `consecutiveFailures` and never `ok`; nothing here writes that tally. ADR 0010's degraded rule is unchanged, asserted by a test.
