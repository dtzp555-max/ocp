...
- **An ABSENT observation and a PASSING observation are indistinguishable unless you first establish that the observation happened** (#405). The entry above is one instance of this; here it is as the general rule, because it showed up three times in one day in three different systems and the third one was not in the test suite at all. The cheap diagnostic is the asymmetry in what a mutation does to the *totals*:

  - **Absorbing** — an earlier assertion catches the mutation aimed at a later one. The suite total drops by one and you see one failure. The lost coverage is invisible, but a signal did arrive.
  - **Aborting** — the mutation kills the process or throws before the later assertions run. Co-located assertions produce **no signal at all**, and **the tests that never ran look identical to tests that passed.**

  **The two have different remedies and were being treated as one thing. Absorption is fixed by ORDERING — put the narrow assertion first, or delete it when a broader one implies it. A process-level abort is fixed only by SEPARATION into independent `test()` registrations; ordering is no defence whatever when the mutation kills the process.**

  Measured, on #405's M3 (drop `shq()` from the `--tools` value). Two claims were pinned — "the value stays one argv element" and "the value cannot break the shell string" — in two separate tests, and the run came back **1193 passed / 2 failed**: the space-bearing value produced an argv diff, and the metacharacter value produced ``/bin/sh: -c: line 0: syntax error near unexpected token `('``. That second failure is a *shell parse abort*: had both claims lived in one test, it would have terminated that test at whichever assertion ran first, the other claim would never have executed, and it would have shipped as proven while reporting a single ordinary-looking failure. Two rows in one run is what proves both are live. (No total is quoted for the co-located case because it was never run — and inventing one would be the "numbers are claims too" defect recorded below, twice over, since merging two registrations into one also changes the denominator.)

  The same shape, one level up in the toolchain, twice more the same day — which is why this is a general rule and not a testing quirk:

  - **A required check that never RAN reads exactly like one that passed.** #405's merge was refused with `Required status check "gitleaks scan (hard fail)" is expected` while all five checks were green on the head SHA. Both readings were true: branch protection gates on the recomputed `refs/pull/N/merge` ref, and a PR landing on `main` in the interval had invalidated it, so the checks had never run *against that ref*. `gh pr checks` showing five passes was true and irrelevant.
  - **A wait loop can exit before the thing it waits for exists.** #404's `until ! gh pr checks | grep -q pending` returned immediately — not because the checks had finished, but because they had not been **created** yet, so there was nothing to match. The loop's negative predicate was satisfied by an empty world.

  **Prescription: require a POSITIVE count before trusting a negative predicate.** Wait for `≥N checks AND 0 pending`, never for `no pending`. Assert `argv.length > 0` and that a known-present element is there *before* asserting any element is absent. In a mutation table, treat "no new failure appeared" as a result only if you can also show the assertions ran — a green control that was never executed is the same defect wearing the opposite colour. This generalises the `''`-slice note above: an empty slice, an empty check list and an unexecuted assertion are the same bug, and each of them satisfies the guard written to catch it.
