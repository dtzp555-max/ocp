...
6. **A declared entry naming a default port (`:443`, `:80`) is kept but flagged at boot.** Measured: `Origin` omits **the port that is default for its own scheme**, and `new URL(origin).host` drops it — `https://ocp.example.com` yields the bare host. So a declaration of `ocp.example.com:443` **will not match an `https://` dashboard**, and the operator gets a 403 telling them to declare a host they *did* declare. The entry is kept exactly as written rather than normalised, because silently dropping the port would widen it to *any* port, which they did not ask for. The boot line says what to write instead. Found by checking the parse against a real `URL`, not by reading the code.

   **It warns and never rejects, because it is a scheme-blind heuristic with a measured false positive.** The default port is *per scheme* and a declaration carries no scheme: `new URL("http://h.example:443").host` **keeps** the port, so an operator serving **plain HTTP on 443** (TLS terminated upstream) has a declaration that works and that this flags. The literals `443`/`80` will not rot — WHATWG's special-scheme table fixes them and OCP only ever sees `http`/`https` — but their *applicability* does, which is the failure condition 规则 5 asks for. Turning the flag into a rejection would break a working deployment on a guess; normalising the port away would silently widen the entry to *any* port, which the operator did not ask for. Keeping it and saying both cases out loud is the only option that is neither. Found by independent review after the first version of this bullet asserted the absolute.
