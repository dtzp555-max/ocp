...
> merge; the measurement commands are given inline so a reader can re-derive them again.
...
Measured against a running server at `eac7c39`, asserting the key **store** and not only the status:
