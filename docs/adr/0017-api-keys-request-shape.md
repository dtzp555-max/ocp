...
**Fix the scalar path with a broad predicate (`isJsonObject`).** ~~Rejected for the reason #360 already recorded at this site: every predicate wide enough to be worth naming also captures inputs this change must not touch. State the requirement positively — the body must be a non-null, non-array object — rather than enumerating what is absent.~~ **Withdrawn — see the second post-sign-off correction below. The implementation uses `isJsonObject`.**

> **Post-sign-off correction 2 (2026-08-11), maintainer-authored.** The struck-through rejection above is wrong, and wrong in two independent ways. The signed text called `isJsonObject` "a broad predicate" and rejected it on the grounds that *"every predicate wide enough to be worth naming also captures inputs this change must not touch."*
>
> **It asserts a property `isJsonObject` does not have.** The predicate and the requirement this ADR prescribes are character-for-character the same condition:
>
> ```js
> const isJsonObject = (v) => typeof v === "object" && v !== null && !Array.isArray(v);   // server.mjs
> //                          the body must be a non-null, non-array object               ← this ADR
> ```
>
> It captures exactly `null | array | string | number | boolean`. Under this ADR the four scalar-or-array forms are precisely the inputs being changed, and `null` keeps the identical status and body it already had — so there is no input it captures that this change must not touch. The property was **inferred from the name**, not from the definition.
>
> **And the reason cited against it is its own origin.** `isJsonObject` *is* #360's implementation of the lesson the rejection invoked; the doc block above it states this ADR's own "state it positively" argument, in the same words, three weeks earlier. Citing #360 to reject #360's answer is the error, not a difference of judgement.
>
> The handler's `// Deliberately NOT isJsonObject` comment was **correct under #360's scope**, which forbade changing which requests are accepted. This ADR removes that constraint, so its premise is gone; it is updated rather than left contradicting the line beneath it. Inlining the condition would also re-open what `server.mjs`'s predicate doc block closes — the two predicates exist so the null case is *"decided ONCE rather than re-remembered at each site"*, and a fourth hand-rolled copy is the fifth reader arriving carrying the bug again.
>
> **Recorded rather than quietly amended, and the process is the point.** The implementer followed the signed rejection literally, wrote the condition inline, and escalated the contradiction instead of substituting the predicate on their own judgement. That was correct: an implementer overriding a signed authorization because they believe it wrong is what the signature exists to prevent, and this ADR being wrong does not retroactively make overriding it right. The fix is the authorization changing, here, in the open.
...
- **Provenance questions in this repo turn on CLONE DEPTH, not on lineage — and the difference decides which answer you get.** ⚠️ *Corrected after sign-off; see the note below.* Asking "when did this line enter?" from the maintainer's working checkout returns `c180987` (2026-07-27), which reads as "it was always there." That was a **shallow-clone artifact**: `.git/shallow` in that checkout held the boundary commit, so history was truncated there and every pre-boundary line looked introduced at the boundary. (That checkout has since been `git fetch --unshallow`ed and now answers correctly; the mechanism below is reproduced from a fresh `git clone --depth=103` rather than from it.) **The boundary commit's own object still knows its parent — only the graph does not**, which is what makes the artifact so convincing: `git log -1 --format=%p <boundary>` prints an empty parent list while `git cat-file -p <boundary>` on the same object shows one `parent` line. And `git merge-base <pre-boundary-sha> origin/main`, with the objects present but the graph truncated, exits **1 with empty stdout and empty stderr** — byte-identical to how git reports a genuine absence of any common ancestor. Nothing distinguishes "there is none" from "I cannot walk far enough to answer". In a full clone the same questions answer correctly and unremarkably — one root (`593d0dc`, 2026-03-16), `v3.16.4` a direct ancestor of `main` (177 commits back as of `eac7c39`), and `git log -S` naming `879b40f`. **Any provenance question in this repo must first establish whether the checkout is shallow** (`ls .git/shallow`, or `git rev-parse --is-shallow-repository`) and, if it is, be re-asked in a full clone or after `git fetch --unshallow`. **`git log --all` is not the cure, and measuring that is what settles it**: in a fresh shallow clone it returns the byte-identical wrong line that `git log -S origin/main` does, because both walk the same truncated graph. It never shows an error, which is why it reads as confirmation.
