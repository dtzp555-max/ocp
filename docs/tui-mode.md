...
> **Correction ([#452](https://github.com/dtzp555-max/ocp/issues/452)).** This paragraph used to contrast `--tools` against "`--allowedTools` / `--disallowedTools`, a **permission** layer". That is accurate for `--allowedTools` and **wrong for `--disallowedTools`**, and the real split is **bare name vs scoped pattern**. Measured from the wire (`claude` 2.1.250, default model, `CLAUDE_CONFIG_DIR` unset, 76-tool unrestricted baseline): `--disallowedTools Bash` → **77 tools, `Bash` absent** — removed from the schema, so it *also* can never prompt; `--disallowedTools 'Bash(git *)'` → **76 tools, `Bash` present** — it stays in the schema and acts at invocation, so it **can**; `--allowedTools Read` → **76 tools, `Bash` present** — pre-approval only. (`77 > 76` is not a typo: removing `Bash` makes the CLI *add* `Glob` and `Grep`.) The **decision** is unaffected — only the reason given for it was wrong.
