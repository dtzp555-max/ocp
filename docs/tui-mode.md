...
`OCP_TUI_TOOLS` default *(unset)*. (TUI-mode) Restrict **which built-in tools** the interactive pane may use, by passing the value straight through to `claude --tools`. This applies to the **default** MCP-walled surface — it is **not** read when `OCP_TUI_FULL_TOOLS=1` (that surface is controlled by `CLAUDE_ALLOWED_TOOLS`, or by its hardcoded default set when that is unset).

Why `--tools` and not `--disallowedTools`: `--tools` is the tool-**availability** registry — it decides which built-in tools *exist* for the session — and it is an **allow**-list, so "only these built-ins" is one flag rather than an enumeration of everything else. That enumeration is the point: a deny-list can only ever deny what its author knew about, which is the staleness [#450](https://github.com/dtzp555-max/ocp/issues/450) was about. A tool that is not in the availability set can never trigger an interactive permission prompt, which matters in a headless tmux pane where nothing can answer such a prompt (an unanswerable prompt hangs the turn to the wallclock cap and bricks the pane — the same failure that rules out `--dangerously-skip-permissions` here).

> **Correction ([#452](https://github.com/dtzp555-max/ocp/issues/452)).** This paragraph used to contrast `--tools` against "`--allowedTools` / `--disallowedTools`, a **permission** layer". That is accurate for `--allowedTools` and **wrong for `--disallowedTools`**, and the real split is **bare name vs scoped pattern**. Measured from the wire — the `tools` array on the **`subtype: "init"`** `system` event of `--output-format stream-json --verbose`; `claude` 2.1.250, default model, `CLAUDE_CONFIG_DIR` unset. Reading the rows under the flags **OCP itself passes here** (`--strict-mcp-config --disallowedTools 'mcp__*'`), because those do not depend on the measuring account's connector set: baseline **27** built-in tools, `Bash` present; `--disallowedTools Bash` → **28**, `Bash` **absent** — removed from the schema, so it *also* can never prompt; `--disallowedTools 'Bash(git *)'` → **27**, `Bash` present — it stays in the schema and acts at invocation, so it **can**; `--allowedTools Read` → **27**, `Bash` present — pre-approval only. (`28 > 27` is not a typo: removing `Bash` makes the CLI *add* `Glob` and `Grep`.) Without `--strict-mcp-config` the same rows read 76 → 77, but **76 = 27 built-in + 49 `mcp__*`** and that 49 is whatever connectors the measuring account has — a number nobody else can reproduce, which is why it is not the one quoted here. The MCP-walled figures are **not** machine-independent either: `CLAUDE_CONFIG_DIR` unset pins the config's *location*, not its *contents*, and the same flags give 27 built-ins unset against **23** with a fresh empty dir. **The split is the claim, not the counts** — and it holds at every baseline measured (76→77, 27→28, 23→24), adding `Glob`+`Grep` each time. The **decision** is unaffected — only the reason given for it was wrong.

- **Value:** comma-separated built-in tool names, e.g. `Read,Glob,Grep,WebSearch,WebFetch`. Per `claude --help` (verified on 2.1.220), `"default"` enables all built-in tools and `""` disables all — but an **empty or whitespace-only** `OCP_TUI_TOOLS` is treated as *unset* (a footgun guard), so it keeps the default rather than silently disabling every tool. The value is shell-quoted as a **single argument**, so whatever you set reaches `claude` verbatim and `claude` — not the shell tmux runs the pane command under — decides how to parse it. That matters for values carrying spaces or shell metacharacters: unquoted, a space word-splits into separate arguments and a scoped name like `Bash(npm run test:*)` is a shell *syntax error*.
...
- **Interaction with `--safe-mode`:** independent. `--safe-mode` (applied on the default surface **when streaming is off** — `OCP_TUI_STREAM` registers a hook via `--settings`, which `--safe-mode` would disable, so the two are mutually exclusive) strips *customizations* — host `CLAUDE.md`, skills, plugins, hooks, MCP — and explicitly leaves built-in tools untouched; `OCP_TUI_TOOLS` is the knob that narrows those built-ins.

> **This is a convenience knob, not a trust boundary.** It narrows what the pane's model is *offered*, on a surface that is already single-operator: TUI refuses to boot under `AUTH_MODE=multi`, and everything the pane does still runs as the OCP host user with that user's credentials. Do not read it as user isolation. ADR 0007 lists `--tools ""` as requirement **1 of 3** for the deferred multi-tenant B-path — the other two being a per-key `HOME` and a sandbox — and this knob supplies only the first. (It also cannot express `--tools ""`: the whitespace-guard above deliberately treats the empty value as *unset*, so a future B-path needs its own call site rather than this env var.)
