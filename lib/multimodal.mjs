...
// ADR 0022 / #477: the image path renders tool turns the SAME way the text path does. Imported
// rather than re-implemented -- a second copy of `[Assistant called tool X with arguments …]` would
// drift from the text path silently, and the whole reason tool results work at all is that the
// wording was measured to turn a re-call into an answer.
//
// NO CYCLE: lib/tool-calling.mjs has no imports at all (checked, not assumed -- `grep -n "^import"`
// on it returns nothing), so nothing reaches back into this module.
import { renderToolTurn, endsWithToolResult, TOOL_CONTINUATION_NOTE } from "./tool-calling.mjs";
...
export const DEFAULT_MULTIMODAL_OPTS = {
  ...
  // #512: the trailing continuation note, as a block after the last tool result. The server turns
  // it off when the note is carried by the system prompt instead (OCP_MULTIBLOCK_INPUT, the default).
  continuationNote: true,
  // #512: a run of consecutive `tool` messages becomes ONE text block. One agent step then adds
  // ~2 blocks however many parallel calls it made; the prompt cache only looks back ~20 blocks
  // from its breakpoint (measured on #512: a hit at 19 appended blocks, a miss at 25).
  coalesceToolResults: false,
};
...
export function buildImageBlocks(messages, opts = {}) {
  ...
  // Pairs each `tool` result with the call that produced it, exactly as messagesToPrompt does.
  const callNames = new Map();
  let prevWasToolResult = false; // #512 coalesceToolResults
  ...
  for (const m of messages) {
    // #477: an assistant message carrying `tool_calls`, or a `tool` message carrying a result, is
    // rendered as TEXT describing what happened -- the same rendering, from the same function, as
    // the text path. Before this, a conversation with an image anywhere was refused tool calling
    // outright, because letting it through meant turn 2 of a vision agent saw the bridge but not
    // its own prior call or the client's result: MEASURED with a real model, it re-called the tool
    // with identical arguments and never saw the result. The rendered turn carries its OWN role
    // marker (`[Assistant] …` / `[Tool X returned]`), so rolePrefix must NOT be applied on top.
    const toolTurn = renderToolTurn(m, callNames);
    if (toolTurn) {
      const sep = firstMessage ? "" : "\n\n";
      firstMessage = false;
      const last = blocks[blocks.length - 1];
      if (o.coalesceToolResults && m.role === "tool" && prevWasToolResult && last && last.type === "text") {
        // Same characters, same order: the separator and the result are appended to the previous
        // result's block instead of opening a new one.
        last.text += sep + toolTurn.text;
        textChars += sep.length + toolTurn.text.length;
      } else {
        pushText(sep + toolTurn.text);
      }
      prevWasToolResult = m.role === "tool";
      // THE TURN'S OWN NON-TEXT PARTS STILL GO THROUGH. renderToolTurn flattens content to text, so
      // an image carried BY a tool result or by the assistant turn that called it would be dropped
      // by a bare `continue` -- and it would be dropped SILENTLY, without even incrementing the
      // image counters. Measured against origin/main, which emits two image blocks for a history
      // whose tool result carries a chart: the first version of this branch emitted one. Found by
      // review; a regression this change introduced, not a pre-existing gap.
      if (Array.isArray(m.content)) {
        for (const part of m.content) {
          if (part && part.type === "image_url") blocks.push(imagePartToBlock(part, o, acc));
        }
      }
      continue;
    }
    prevWasToolResult = false;
  ...
  // #477: the continuation note, once, after the last rendered result -- the instruction that was
  // measured to make the model ANSWER from the supplied result instead of calling again. The text
  // path appends it to the joined string; here it is its own trailing text block, which is the same
  // position in the same order.
  if (o.continuationNote && endsWithToolResult(messages)) pushText(`\n\n${TOOL_CONTINUATION_NOTE}`);
...
export function buildStreamJsonInput(messages, opts = {}) {
  ...
  // #512: one cache breakpoint on the LAST block this request sends. The CLI's own final breakpoint
  // lands on a block it appends after the client's content, so the entry it writes is never a prefix
  // of the next request; an entry that ends on OUR last block is, and the next request finds it by
  // the API's short lookback. Measured on opus 5.5 (#512, 2026-09-25): each agent step then reads the
  // whole previous prompt and writes only the new blocks.
  if (opts.cacheBreakpointTtl && blocks.length) {
    blocks[blocks.length - 1] = { ...blocks[blocks.length - 1], cache_control: { type: "ephemeral", ttl: opts.cacheBreakpointTtl } };
  }
  const envelope = { type: "user", message: { role: "user", content: blocks } };
  // blockCount (#512): how many content blocks the prompt cache sees for this request.
  return { payload: JSON.stringify(envelope) + "\n", stats: { ...stats, blockCount: blocks.length } };
}
