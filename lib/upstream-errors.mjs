...
const RATE_LIMIT_PATTERNS = [
  ...
  // The five siblings, added on the owner's decision 2026-09-19 (#493). Verb-anchored for the
  // same reason "hit your session limit" is: the bare nouns are settings-UI labels. See below.
  "hit your fast limit",
  "hit your monthly limit",
  "hit your monthly spend limit",
  "hit your channel's monthly spend limit",
  "hit your team's shared budget",
...
// THAT LIST OF THREE IS A MEASUREMENT ON 2.1.270 AND IS LEFT AS ONE. The 2026-09-19 sweep of
// 2.1.277 found FIVE (adding `team's shared budget` and `monthly limit`). Annotated rather than
// corrected, because editing the figure would falsify a dated reading of a different build --
// the same discipline the `probesTuiPool` 16-vs-14 note in CLAUDE.md applies.
//
// THE SIBLINGS ARE NOW IN THE LIST, and the count was never three. Added on the owner's decision
// of 2026-09-19 (#493), after the negative sweep that issue asked for. What the sweep changed:
//
//   1. There are FIVE wall phrasings, not three. `team's shared budget` and `monthly limit` were
//      not in #493's list at all. Measured against this classifier, all five returned false --
//      i.e. all five were reaching callers as 500.
//   2. THE TEMPLATE OPTION IS OUT, on evidence rather than caution. #493 proposed matching
//      `hit your ` plus a limit noun and warned the prefix might be shared with non-quota text.
//      It is: the binary carries
//        client composes its own "You've hit your limit" line and drops your message;
//      which is prose ABOUT the behaviour, not a wall. A bare `hit your ` -- or a bare
//      `hit your limit` -- matches it.
//   3. TWO OF THE BARE NOUNS ARE DEMONSTRABLY UNSAFE. `monthly spend limit` alone appears 23
//      times in the binary and most are SETTINGS UI:
//        Adjust monthly spend limit:      Set your monthly spend limit to
//        Removed monthly spend limit      Increased monthly spend limit to
//        org's monthly spend limit        /usage-credits to adjust your monthly spend limit.
//      `monthly limit` alone is the same shape (`Adjust monthly limit`). Promoting a settings
//      label to 429 is the fail-OPEN direction -- it tells a fallback-capable client to leave the
//      vendor over a menu item. Test rows below pin these as non-matches, and widening either
//      pattern back to its bare noun reddens them by name.
//
// THE OTHER THREE ANCHORS ARE CONSISTENCY, NOT MEASUREMENT, and saying so is the point. In this
// binary `fast limit` occurs twice and `team's shared budget` eight times, and EVERY occurrence of
// both is inside a wall -- so for those two the bare noun is not demonstrably unsafe here, and the
// bare-noun mutation produces NO red. They carry the verb anyway because of what builds the
// message, which is the stronger argument and is structural rather than statistical:
//
//   function Ry(e,n,r,s){ … return `You've hit your ${e}${n}${g}` }
//
// Every wall is that template with a limit NAME substituted, so the verb is present in all of
// them by construction, while the noun on its own is just a label the same binary also renders in
// menus. It also means the limit-name set is OPEN -- supplied at runtime, not enumerable from the
// binary -- which is why this list matches completed phrasings rather than trying to be exhaustive.
// `channel's monthly spend limit` needs its own entry for the same reason the anchor matters at
// all: the possessive breaks it, and the noun it wraps is one of the two unsafe ones.
//
// CORRECTION (#505, 2026-09-20). The paragraph that stood here said `org's monthly spend limit` appears
// "only as a SETTINGS LABEL, never inside a wall". THAT WAS WRONG, and it was wrong in the
// direction that argued against adding it. It is a limit NAME passed to the wall builder:
//
//   let Le = s ? "monthly spend limit" : _e ? QFt : "org's monthly spend limit";
//   return Ry(Le, `${He}${Ne}`, n)                  // and the sibling site:
//   return Ry(_e ? QFt : "org's monthly spend limit", `${We}${Ne}`, n)
//
// so it renders `You've hit your org's monthly spend limit` -- a wall. (A first draft of THIS
// correction cited the `oPr` site instead; with `oPr = "individual usage limit"` that ternary can
// only render the two USAGE walls, never the org spend one. Caught in review. The conclusion held,
// the evidence attached to it did not -- the same failure this paragraph retracts, committed
// inside the retraction. Spend goes through QFt, usage through oPr; different call sites.)
// The original error came from reading
// a bare `strings` hit and inferring its role from the strings AROUND it, instead of finding what
// CONSUMES it. The standalone hit and the settings labels look identical in a `strings` dump; only
// the call site tells them apart.
//
// THE LIMIT-NAME SET IS NOW TAKEN FROM Ry's OWN CONSTANTS (#505), a better source than a string
// sweep because it is the argument table rather than a grep:
//
//   oPr = "individual usage limit"        iPr = "channel's monthly usage limit"
//   QFt = "individual spend limit"        sPr = "channel's monthly spend limit"
//   plus the inline literals: "org's monthly usage limit", "org's monthly spend limit",
//   "team's shared budget", "monthly spend limit", "monthly limit", "fast limit"
//
// ONE WALL IS KNOWINGLY LEFT AT 500, named here so it is not rediscovered as a bug. `Ry("limit", …)`
// is a real call site, so `You've hit your limit` is a genuine wall. It is NOT added, because the
// only anchor it could carry -- `hit your limit` -- also matches
//     client composes its own "You've hit your limit" line and drops your message;
// which is prose about the behaviour and is pinned as a NEGATIVE row. Covering the wall and
// excluding the prose cannot both be done with a substring, so this one fails CLOSED -- the
// direction this list is designed to fail in. It needs a whole-message match, not one more pattern.
//
// Measured against this list on v3.37.2, 9 of 11 rendered walls classified; the two that did not
// were `individual spend limit` and `org's monthly spend limit`, both added above. The set is
// still OPEN -- the name is substituted at runtime -- so this is coverage of what 2.1.277 can
// render, not a closed enumeration.
//
// `channel's` needs its own entry because the possessive breaks the anchor: "You've hit your
// channel's monthly spend limit." does not contain "hit your monthly spend limit".
//
// ASCII APOSTROPHE, and that is measured rather than assumed: the binary contains 29 863 ASCII
// apostrophes and ZERO U+2019 -- verified with a positive control on a known-U+2019 input first,
// because a grep that cannot see the character reports the same zero as a binary that lacks it.
//
// EXPIRY: this corpus is a property of `claude` 2.1.277 (a native Mach-O; `strings -a` + grep,
// not `grep cli.js`). Re-run the sweep with a PERMISSIVE character class against whatever version
// is installed -- a restricted class returned 9 of the 16 `hit your ` variants here and hid both
// of the decisive ones.
//
// `usage limit` IS A BARE NOUN, AND IT IS LOAD-BEARING (#505) -- the opposite of what the
// previous version of this note claimed. It called `org's monthly usage limit` matching it a
// "pre-existing false positive". It is not a false positive: that string is a WALL (see the
// CORRECTION block further down this comment), and the bare pattern is the only reason THREE
// real walls classify today --
//
//   You've hit your individual usage limit          (oPr)
//   You've hit your org's monthly usage limit
//   You've hit your channel's monthly usage limit   (iPr)
//
// -- none of which has a verb-anchored entry of its own. So do NOT narrow `usage limit` on the
// strength of that old note: doing so would silently return those three to 500. They now have
// POSITIVE test rows pinning them, so the narrowing reddens instead of passing quietly.
//
