...
//
// CORRECTION (2026-09-20). The paragraph that stood here said `org's monthly spend limit` appears
...
//
//   return Ry(e.overageDisabledReason === "org_spend_cap_reached" ? oPr : "org's monthly usage limit", Ee, n)
//
// so it renders `You've hit your org's monthly spend limit` -- a wall. The error came from reading
...
//
// THE LIMIT-NAME SET IS NOW TAKEN FROM Ry's OWN CONSTANTS, which is a better source than a string
...
//
// `usage limit` IS A BARE NOUN, AND IT IS LOAD-BEARING -- which is the opposite of what the
...
// correction below), and the bare pattern is the only reason THREE real walls classify today --
//
