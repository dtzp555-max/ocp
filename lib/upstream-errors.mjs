...
//
// ONLY `channel's` IS HANDLED, AND THE GAP IS NAMED RATHER THAN IMPLIED: any OTHER possessive
// limit name breaks the anchor the same way. `You've hit your org's monthly spend limit` matches
// none of the five and would still surface as 500. That is knowingly best-effort, because the
// limit-name set is supplied at runtime and cannot be enumerated from the binary -- and in this
// build `org's monthly spend limit` appears only as a SETTINGS LABEL, never inside a wall, which
// is why it is not added on the strength of a shape. If an `org's` wall is ever observed, it is
// one more entry, added the same way these five were.
//
...
//
// STILL OPEN, deliberately unfixed here: `usage limit` is a bare noun of exactly the kind this
// entry argues against, and `org's monthly usage limit` (a settings label in the same binary)
// matches it TODAY. That predates #493 and narrowing the oldest pattern in the list is its own
// change with its own negative sweep; recorded so the asymmetry is visible rather than implied.
//
