...
// #512: the same instruction, worded as a CONDITION so it can live in the system prompt, where it is
// byte-constant for the whole conversation. As a trailing user-turn block it moved to the new end of
// the prompt on every tool step, and the block the previous request's cache entry ended on was then
// never a prefix of the next request -- measured to re-write the whole conversation into the cache
// every step (#512 plan, row 4). The wording keeps the measured instruction's substance: the calls
// were made by you, executed by the client, the results are final, answer if they are enough.
export const TOOL_CONTINUATION_SYSTEM_NOTE =
  "When the conversation ends with the results of tool calls you made (each shown as " +
  "\"[Tool <name> returned]\" followed by its output), those calls were executed by the client and " +
  "their results are final. Continue from there: if the results are enough, answer the user " +
  "directly. Only call a tool again if you need something not already returned.";
