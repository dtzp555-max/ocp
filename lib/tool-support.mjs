...
/**
 * How many tools a request DECLARED, counting both the current and the deprecated form (#467).
 *
 * WHY THIS EXISTS SEPARATELY FROM classifyToolRequest. That function answers "must OCP refuse this
 * request", and per ADR 0013 it refuses exactly the four shapes the OpenAI specification OBLIGES a
 * tool call for. Everything else -- `tool_choice` absent, `"none"`, `"auto"`, or
 * `allowed_tools.mode: "auto"` -- is satisfiable by a text answer, so it is let through unchanged.
 *
 * That let-through path is where #467 lives: the declared tools are silently DROPPED, the client
 * gets `finish_reason: "stop"` and HTTP 200, and an agent framework pointed at OCP degrades into a
 * chatbot with every signal on both sides green. It is the case ADR 0013 explicitly declined to
 * answer, and it has now cost hours twice.
 *
 * This counter does NOT change what is refused. ADR 0013's Alternatives rejected "refuse whenever
 * `tools` is present" because it would have taken down every OpenClaw agent on the fleet the day it
 * shipped -- they all send tools and all accept text -- and ADR 0021 keeps that constraint. The
 * request is answered exactly as before; it just stops being invisible.
 *
 * Counts REQUESTS' declared tools, not calls: a request declaring 9 tools returns 9.
 *
 * @param {unknown} parsed a parsed /v1/chat/completions body
 * @returns {number} how many tools were declared, 0 if none or if the field is malformed
 */
export function countDeclaredTools(parsed) {
  let n = 0;
  // The current form.
  if (Array.isArray(parsed?.tools)) n += parsed.tools.length;
  // The DEPRECATED form, counted for the same reason classifyToolRequest checks `function_call`:
  // a client using the old spelling has the identical silent failure, and leaving it uncounted
  // would make the counter read 0 on exactly the setup least likely to be updated.
  if (Array.isArray(parsed?.functions)) n += parsed.functions.length;
  return n;
}
