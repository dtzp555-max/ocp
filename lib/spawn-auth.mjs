...
//
// A LIST, not four `delete` lines per site. There were two scrub sites (the auth probe and the
// spawn path) already drifting in purpose, and a denylist you have to remember to extend at both
// is the failure mode that produced this bug. Adding a new inbound-auth env var means adding it
// HERE, once — `scrubInboundAuthEnv` is the only writer both sites call.
//
