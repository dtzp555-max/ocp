...
export function applyRequestVerdictTtl(status, now, ttlMs) {
  if (!status || status.lastOutcome !== "verified-by-request") return status;
  // Number.isFinite, not `typeof === "number"`: NaN IS a number, and every comparison against it
  // is false, so `now - NaN <= ttlMs` falls through to the decay and a NaN timestamp silently
  // expires the verdict. Caught by the test below, which is the whole reason it exists — this is
  // the same NaN-passes-every-threshold shape that has bitten before, written wrong again here.
  if (!Number.isFinite(status.lastCheck) || now - status.lastCheck <= ttlMs) return status;
  return { ...status, ok: null, message: "last successful request is older than the verification window" };
}
