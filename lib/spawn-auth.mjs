...
export function applyRequestVerdictTtl(status, now, ttlMs) {
  ...
  // is false, so `now - NaN <= ttl` falls through and a malformed timestamp silently expires the
  // verdict. Written wrong here once already; the test below is why it did not ship.
  if (!Number.isFinite(status.okAt) || now - status.okAt <= ttlMs) return status;
