...
//
// Only a REQUEST-established verdict decays — `okSource === "request"`. A probe verdict is the
// probe's business and is governed by its own rules; this function must not silently rewrite one.
// (This line used to say `Only "verified-by-request" decays`, naming a lastOutcome value the
// server has never emitted, three lines above the comment below that says not to key on
// lastOutcome. #342.)
