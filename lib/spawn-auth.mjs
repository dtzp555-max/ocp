...
//
//   server.mjs   :166 where.exe lookup   :173 which lookup   :1169 auth probe
//                :1500 the -p spawn      :2436 keychain read
//   lib/tui/     :442 pane `env -u` prefix (aliases)   :646 tmux server env
//   setup.mjs    :71 which lookup   :153 claude --version   :177 the -p auth probe
//
// Six of those are pinned by a test that fails when that one site is reverted. The three
// binary-lookup/keychain sites in server.mjs are NOT — they are scrubbed for consistency, none
// of them reads attacker-controlled text, and server.mjs cannot be imported by the suite. Stated
// rather than glossed: an inventory whose coverage is asserted but untrue is how the third and
// fourth spawn sites stayed hidden.
//
