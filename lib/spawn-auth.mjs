...
// writer, and every site calls it: server.mjs's auth probe and `-p` spawn, `bootTuiPane`'s tmux
// server env, `buildTuiCmd`'s pane `env -u` prefix, and setup.mjs's two spawns.
//
...
export function scrubInboundAuthEnv(env) {
  ...
  if (secrets.size > 0) {
    for (const name of Object.keys(env)) {
      if (typeof env[name] === "string" && secrets.has(env[name])) { delete env[name]; removed.push(name); }
    }
  }
  return { env, removed };
}
