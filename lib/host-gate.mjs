...
export function isRebindSafe(authority) {
  ...
  const h = authority.hostname.replace(/\.$/, "");
...
export function parseAllowedHosts(raw) {
  ...
  for (const raw_tok of String(raw == null ? "" : raw).split(",")) {
    ...
    // MEASURED, not reasoned: `Origin` never carries a DEFAULT port, and `new URL(origin).host`
    // drops it — `https://ocp.example.com` yields the bare host. So a declaration of
    // `ocp.example.com:443` can never match the origin the operator is trying to admit, and they
    // get a 403 saying "declare this host" for a host they DID declare. The entry is kept exactly
    // as written (silently dropping the port would widen it to any port, which they did not ask
    // for); the caller reports it so the fix is a sentence rather than a debugging session.
