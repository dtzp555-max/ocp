...
//   every other branch       `--allowedTools Bash Read Write ...`                       -> 27 tools
...
//
// EXPIRY: if buildCliArgs gains a fourth branch with a different tool surface, this gains a fourth
// case. That is not left to vigilance — the integration tests assert the prompt and the tool flags
// FROM THE SAME SPAWN, so adding a branch there without one here reddens.
