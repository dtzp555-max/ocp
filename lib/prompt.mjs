...
// asserts the prompt and the flags from the same spawn, so those three cannot drift SILENTLY -- the
// deepEqual reddens and forces a person to look. WHETHER THE WRAPPER THEN FOLLOWS IS STILL THAT
// PERSON'S JOB, which is the same category as the fourth-arm case below, and an earlier revision of
// this sentence said "cannot drift" full stop. A review falsified it with the coherent edit a
// developer actually makes: change the skip-permissions arm to `--tools ""` AND update that test's
// deepEqual -- because the test reddened and told them to -- while leaving this function alone.
// Result: the co-location set stayed 10 passed / 0 failed, with that arm emptying the schema
// while its spawn is handed the
// neutral "tools granted" wrapper. The reason is structural: TOOLS_GRANTED is derived from
// AUTH_MODE, never from the argv buildCliArgs actually produced, so no test relates the two.
// (Deriving it from the argv would close this. That is a design change, not a comment fix.) A FOURTH arm
