...
//   streamTurns       — streamed turns attempted
//   streamDeltas      — hook fires forwarded to clients
//   streamTopUps      — turns where the delta stream was a safe PREFIX of the transcript but
//                       not equal to it; OCP topped up from the transcript and served T. Benign
//                       but worth watching — a persistent rate means the hook is losing fires.
//   streamDivergences — turns REFUSED because emitted bytes were not a prefix of the
//                       transcript. THE field to alert on: it means the hook and the transcript
//                       disagreed and OCP chose to fail rather than serve unverifiable text.
