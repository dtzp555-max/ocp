...
export function buildTuiCmd(claudeBin, model, sessionId, ehome, entrypointMode, stream = null) {
  ...
  if (process.env.OCP_TUI_FULL_TOOLS === "1") {
    ...
    // [measured] wire instrument (the `tools` array on the `system` init event of
    // --output-format stream-json --verbose), claude 2.1.250, default model, CLAUDE_CONFIG_DIR
    // unset, against a 76-tool unrestricted baseline:
    //
    //   --disallowedTools Bash            -> 77 tools, Bash ABSENT   removed from the schema, so
    //                                                                it also cannot prompt
    //   --disallowedTools 'Bash(git *)'   -> 76 tools, Bash present   stays in the schema and acts
    //                                                                at invocation, so it CAN
    //   --allowedTools Read               -> 76 tools, Bash present   pre-approval only
    //
    // (77 > 76 is not a typo: removing Bash makes the CLI ADD Glob and Grep.)
    //
