...
export function buildTuiCmd(claudeBin, model, sessionId, ehome, entrypointMode, stream = null) {
  ...
  if (process.env.OCP_TUI_FULL_TOOLS === "1") {
    ...
    // because these figures reproduce on any machine:
    //
    ...
    // the flags alone and is deliberately not the number quoted here. EXPIRY: re-run the probe;
    // if the built-in count has moved, the counts are stale -- the bare-vs-scoped SPLIT is the
    // claim, and it is what a re-measurement has to confirm or refute.
    //
