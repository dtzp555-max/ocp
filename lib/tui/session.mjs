...
export async function runTuiTurn({
  ...
  // #362: from here until the `finally` below, this pane is held by a turn and is reachable by
  // nothing else — see liveTurnPanes above. Registered HERE rather than inside the `try`, because
  // the tmux session is already LIVE at this point (bootTuiPane creates it synchronously, and a
  // warm pane has been live since its own boot), so a shutdown landing on the next line must
  // still be able to find it.
  //
  // NOT A CLAIM THAT THE MAP IS ALWAYS DRAINED, and the gap is stated rather than hidden: the
  // `try` opens after the mkdtempSync/writeFileSync below, so a throw from either leaves this
  // entry behind. That window is a PRE-EXISTING pane leak — the `finally` does not cover those
  // two statements either, so the pane already outlives the turn there today — and registering
  // makes it strictly better, because shutdown now kills a pane that previously nothing owned.
  // The residue is one Map entry per occurrence. Closing the window properly means moving those
  // statements inside the `try`, which changes teardown behaviour for a different defect and
  // belongs to its own PR.
  liveTurnPanes.set(tmuxName, tmux);
