...
//
// STREAMING (OCP_TUI_STREAM, default off). Pass `onDelta` and — for the COLD path — `streamDir`,
// and the pane's MessageDisplay hook (installed by bootTuiPane; see lib/tui/stream.mjs) appends
// each raw delta payload to the pane's own sink. This driver polls that sink and invokes
// onDelta(payload) per fire while the turn is still generating. A WARM pane already carries its
// sink from boot (pane.streamFile), so the pooled and cold paths stream identically.
//
...
// agree. With onDelta omitted, nothing here changes: no poll, and (given no streamDir) no hook.
//
...
export async function runTuiTurn({
  ...
  streamDir = null,  // hook sink dir, for the COLD boot below (a warm pane brings its own)
  abortSignal = null,
}) {
  ...
  if (!pane) {
    pane = await bootTuiPane({ model, claudeBin, home, realHome, cwd, port, entrypointMode, tmux,
                              streamDir: onDelta ? streamDir : null });
  }
