...
import { INBOUND_AUTH_ENV_VARS, scrubInboundAuthEnv } from "../spawn-auth.mjs";
...
export function buildTuiCmd(claudeBin, model, sessionId, ehome, entrypointMode, stream = null) {
  ...
  // #328: `...INBOUND_AUTH_ENV_VARS` is not decoration — this was the THIRD copy of the four-name
  // list above, and it is the ONLY request path when CLAUDE_TUI_MODE=true (server.mjs:3113/:3229
  // pick callClaudeTui over callClaude wholesale). A fix applied only to `spawnClaudeProcess`
  // leaves every TUI request unprotected, and the claim "a new inbound-auth variable is added in
  // one place" false. Sourced from lib/spawn-auth.mjs so it stays true.
  // #328 review-4 P2: the prefix is a list of NAMES, so it cannot express "whatever variable
  // happens to carry the credential". When OCP starts the tmux server the pane is covered by the
  // server-env scrub below; when a FOREIGN server is already running that scrub never applies and
  // this prefix is the only defence. So compute the aliases too — scrubInboundAuthEnv reports
  // exactly which names it removed from a copy of the environment, and those names go on the list.
  const aliased = scrubInboundAuthEnv({ ...process.env }).removed;
  const unset = [...new Set(["CLAUDECODE", "ANTHROPIC_API_KEY", "ANTHROPIC_BASE_URL", "ANTHROPIC_AUTH_TOKEN",
                             ...INBOUND_AUTH_ENV_VARS, ...aliased])];
  ...
  // shq() on the name, not just the value. Before this PR `unset` held four string literals; the
  // alias names spliced in above are the first RUNTIME-derived value to reach this string, and
  // every other runtime value here already goes through shq(). A reviewer demonstrated the gap
  // end to end: an env var whose NAME contains shell metacharacters produced a pane command that
  // executed them. Low exploitability — it needs a metacharacter-bearing name in OCP's OWN
  // environment, which the injected child cannot set — but the fix is one token.
  const envPrefix = ["env", ...unset.map((u) => `-u ${shq(u)}`), ...sets].join(" ");
  ...
  if (process.env.OCP_TUI_FULL_TOOLS === "1") {
    ...
    // --tools is the tool-AVAILABILITY registry (which built-in tools exist for the session), so a
    // tool omitted here is never offered and can never trigger an interactive permission prompt
    // that a headless tmux pane cannot answer (which would hang the turn to the wallclock cap and
    // brick the pane).
    //
    // #452: an earlier revision drew that contrast against `--allowedTools / --disallowedTools` as
    // a whole, calling them "a permission layer". That is right for --allowedTools and WRONG for
    // --disallowedTools, and the split is bare-name vs SCOPED rather than one flag vs another.
    // [measured] wire instrument: the `tools` array on the system event with subtype "init"
    // (NOT merely the first `type:"system"` line -- a SessionStart hook emits `subtype:
    // "hook_started"` ahead of it, and selecting on type alone silently reads an event with no
    // tools array at all). claude 2.1.250, default model, CLAUDE_CONFIG_DIR unset.
    //
    // Rows taken UNDER THE FLAGS THIS BRANCH ITSELF PASSES -- --strict-mcp-config and
    // --disallowedTools mcp__*, both unconditional a few lines above -- because a measurement
    // taken under a configuration this call site never produces is the wrong instrument, and
    // because walling MCP off drops the dependence on the measuring ACCOUNT'S CONNECTOR SET:
    //
    //   baseline                          -> 27 tools, Bash present
    //   --disallowedTools Bash            -> 28 tools, Bash ABSENT   removed from the schema, so
    //                                                                it also cannot prompt
    //   --disallowedTools 'Bash(git *)'   -> 27 tools, Bash present   stays in the schema and acts
    //                                                                at invocation, so it CAN
    //   --allowedTools Read               -> 27 tools, Bash present   pre-approval only
    //
    // (28 > 27 is not a typo: removing Bash makes the CLI ADD Glob and Grep.)
    //
    // WITHOUT --strict-mcp-config the same rows read 76 -> 77. That 76 is 27 built-in plus 49
    // mcp__* from whatever connectors the MEASURING ACCOUNT has, so it is not reproducible from
    // the flags alone and is deliberately not the number quoted here.
    //
    // 27 IS NOT MACHINE-INDEPENDENT EITHER, and an earlier revision of this comment said it was
    // -- F4's own defect one level down, inside F4's fix. CLAUDE_CONFIG_DIR *unset* pins the
    // config's LOCATION, not its CONTENTS, and on another host unset resolves to that operator's
    // ~/.claude, which differs by construction. Measured, same machine, same account, same
    // claude, same flags, varying only CLAUDE_CONFIG_DIR: unset -> 27 built-ins; a FRESH EMPTY
    // dir -> 23 (LSP, Monitor, PushNotification, RemoteTrigger drop out).
    //
    // THE SPLIT IS THE CLAIM; THE COUNTS ARE THE READING THAT PRODUCED IT. It has now held at
    // three independent baselines -- 76 -> 77, 27 -> 28, 23 -> 24 -- each adding Glob and Grep.
    // EXPIRY: re-run the probe; if a count has moved that is expected, and only a baseline whose
    // bare-name row does NOT drop the tool refutes anything.
    //
    // NONE OF THIS CHANGES THE DECISION. --tools is still the right flag here because it is an
    // ALLOW-list: expressing "only these built-ins" with --disallowedTools would mean enumerating
    // everything else, which is exactly the deny-list staleness #450 was about. What changed is
    // the stated reason, which is the same shape as #451's own F1 -- the choice was right and the
    // sentence justifying it was not. Value is passed through verbatim (comma-separated names, e.g.
    // "Read,Glob,Grep,WebSearch,WebFetch"; "" disables all, "default" enables all — per
    // `claude --help`, verified on 2.1.220). Unset or empty => today's behaviour: all
    // built-in tools available, MCP walled off. shq'd as ONE arg so that claude — not the shell
    // tmux runs this string under — decides how to parse it: an unquoted value with a space is
    // word-split into separate argv elements, and a scoped/globby name like "Bash(npm run x:*)"
    // is a shell SYNTAX ERROR. Both are pinned behaviourally in test-features.mjs (#190).
...
// The TURN pane's name shape — the same port-scoped prefix, WITHOUT the pool's "p" marker.
// Extracted (#362) because it now has two callers: bootTuiPane's default, and runTuiTurn, which
// must name a cold pane BEFORE booting it so shutdown can reach it during the boot. Two copies of
// this formula that disagreed would be silent and severe — the registry would name a session that
// does not exist while the live one stayed unowned, i.e. the exact orphan the registration exists
// to prevent. One function means they cannot disagree.
export function turnPaneName(port, sessionId) {
  return sessionPrefixForPort(port) + sessionId.slice(0, 8);
}
...
export async function bootTuiPane({
  ...
  const tmuxName  = name || turnPaneName(port, sid);
  ...
  // #328: scrub the TMUX SERVER's own environment too, not just the pane's `env -u` prefix.
  //
  // buildTuiCmd's own note ("tmux does NOT forward the spawning process's environment to the
  // pane", verified live 2026-06-01) is CORRECT and is not being contradicted here — it describes
  // an `spawnSync` against an ALREADY-RUNNING tmux server, where the pane inherits the server's
  // environment and this `{env}` is irrelevant. What it does not cover is the other case: when
  // this call is what STARTS the server, the server takes this environment, and every later pane
  // inherits it. Both were measured. That gap is why this site went unexamined through the whole
  // of #328's analysis.
  //
  // And "a clean server is usually already running" is not a mitigation, it is the opposite: OCP
  // calls `tmux kill-server` in its own boot reap (see the reap logic above), so OCP guarantees
  // the next `new-session` rebuilds the server under THIS environment. The leak is the default
  // path, not an edge case.
  //
  // Worth scrubbing even with the pane prefix in place: the tmux server is long-lived and its
  // /proc/<pid>/environ is readable by any same-UID process — which includes a spawned claude.
...
// ── In-flight turn panes: the ownership gap shutdown could not see (#362) ────────────────
// A pane that is SERVING A TURN is owned by nothing `gracefulShutdown` can reach. A warm pane
// LEAVES the pool the moment it is acquired (that is POOL/REAPER INVARIANT property 2 working
// as designed — an acquired pane is the CALLER's, and the pool must not also claim it live); a
// cold-booted pane was never in the pool at all; and neither is in server.mjs's
// `activeProcesses`, because a pane's `claude` is a child of the tmux SERVER, not of node. So
// shutdown drained the pool, observed `activeProcesses.size === 0`, and called `process.exit(0)`
// — leaving a live, interactive, AUTHENTICATED `claude` behind. `runTuiTurn`'s own `finally`
// would have killed it, and never got to run.
//
// This registry is the missing half. It is deliberately NOT the universal "active-work registry"
// the external audit prescribed: that program is rejected by ADR 0015, whose correction 5 records
// that this issue has a much smaller seam. The seam is that the module which already owns panes,
// and already kills them, can also NAME the ones a turn is holding.
//
// SYNCHRONOUS BY CONSTRUCTION, which is a hard requirement and not a style choice.
// `gracefulShutdown`'s step 2b comment already states it for the pool: `process.exit(0)` runs in
// THAT SAME TICK whenever `activeProcesses` is empty — which on a TUI host it always is — so any
// cleanup deferred to a `.then()`/`.catch()` would simply never run. That was already a real bug
// once: the pool used to track in-flight boots as a COUNT, could not name the booting session,
// and orphaned a live authenticated `claude` on every shutdown that landed mid-boot. A future
// reader who does not know that will write the deferred version and it will silently do nothing.
//
// Keyed by tmux session name, valued by THIS turn's own `tmux` function — so an injected fake
// (tests) is the one invoked, and a harness can never reach a real tmux server through here.
const liveTurnPanes = new Map();

// Issue `kill-session` for every pane a turn is currently holding.
//
// Returns how many registered panes a kill was ISSUED for. That is NOT a claim that each died:
// `tmux kill-session` on an already-dead session exits non-zero, which is expected and ignored
// here exactly as it is in `runTuiTurn`'s own teardown. Callers use the count for logging only.
//
// Safe to call unconditionally. With TUI mode off, `runTuiTurn` is never entered, the map is
// empty, and this performs ZERO tmux invocations — so it is deliberately NOT gated on a TUI
// flag. A gate is one more thing that can be wrong, and an empty map already gives the same
// guarantee by construction.
export function killLiveTurnPanes() {
  let issued = 0;
  for (const [name, tmuxFn] of liveTurnPanes) {
    try { tmuxFn(["kill-session", "-t", name]); issued++; } catch { /* already gone */ }
  }
  liveTurnPanes.clear();
  return issued;
}
...
export async function runTuiTurn({
  ...
  // #362: take ownership BEFORE the pane can exist unowned. Which moment that is differs by path,
  // and getting it wrong on the cold path was this fix's own first defect (found in review):
  //
  //   WARM — the pane has been live since its own boot, and `acquire()` above already dropped it
  //          from the pool. It is ours from this line, with no window at all.
  //   COLD — `bootTuiPane` creates the tmux session SYNCHRONOUSLY, and then AWAITS readiness for
  //          up to BOOT_MS (4 s by default). Registering after that await left the pane live,
  //          unpooled and unregistered for the whole window — measured: a SIGTERM landing inside
  //          the readiness poll produced no kill-session at all, deterministically, on the
  //          default configuration (OCP_TUI_POOL_SIZE=0, i.e. cold boot is the DEFAULT path).
  //          So mint the identity FIRST and register it, exactly as lib/tui/pool.mjs does for a
  //          booting pool pane ("Mint the identity BEFORE booting"), which is the same defect
  //          this file's own history already records the pool having had and fixed.
  if (pane) {
    liveTurnPanes.set(pane.name, tmux);
  } else {
    const sid = randomUUID();
    const coldName = turnPaneName(port, sid);
    liveTurnPanes.set(coldName, tmux);
    try {
      // streamDir passed AS-IS (not gated on onDelta) — F4: see the STREAMING comment above.
      // sessionId/name are supplied so the pane's identity is the one already registered above;
      // bootTuiPane accepts both for exactly this reason (the pool is its other caller).
      pane = await bootTuiPane({ model, claudeBin, home, realHome, cwd, port, entrypointMode, tmux,
                                 streamDir, sessionId: sid, name: coldName });
    } catch (e) {
      // Nothing is owned any more: `tui_spawn_failed` means new-session never created a session,
      // and the readiness throw (requireReady, which the cold path does not set) kills the pane
      // itself before throwing. Leaving the entry would name a session that cannot exist.
      liveTurnPanes.delete(coldName);
      throw e;
    }
    // Forward guard, [reasoned] not [measured] — no mutation in this PR produces this shape.
    // Unreachable while bootTuiPane honours a supplied `name` (`name || turnPaneName(...)`), but
    // if that ever changes, re-point rather than throw: a wrong entry means the registry names a
    // dead session while the live one is unowned, which is the orphan this whole block prevents.
    if (pane.name !== coldName) {
      liveTurnPanes.delete(coldName);
      liveTurnPanes.set(pane.name, tmux);
    }
  }
  const tmuxName  = pane.name;
  // NOT A CLAIM THAT THE MAP IS ALWAYS DRAINED, and the residue is stated rather than hidden: the
  // `try` below opens after the mkdtempSync/writeFileSync, so a throw from either leaves this
  // entry behind. That window is a PRE-EXISTING pane leak — the `finally` does not cover those two
  // statements either, so the pane already outlives the turn there today — and registering makes
  // it strictly better, because shutdown now kills a pane that previously nothing owned. The
  // residue is one Map entry per occurrence. Closing it properly means moving those statements
  // inside the `try`, which changes teardown behaviour for a different defect and is its own PR.
  ...
  try {
    ...
    streamStopped = true;
    if (pollTimer) clearTimeout(pollTimer);
    // #362: hand ownership back BEFORE the kill below. Ordering is not load-bearing for
    // correctness (both paths issue the same idempotent kill-session and shutdown's copy is
    // caught), but de-registering first means the map never names a pane this turn has already
    // decided to tear down.
    liveTurnPanes.delete(tmuxName);
    try { tmux(["kill-session", "-t", tmuxName]); } catch { /* already gone */ }
