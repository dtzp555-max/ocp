...
// Hard cap on OCP_TUI_POOL_SIZE. Each pane is an idle claude process, so a small host
// (a Pi serving a family) still wants a small pool — but the operator already controls
// that with OCP_TUI_POOL_SIZE itself, so a low hard cap protected nobody who couldn't
// already protect themselves, while it stopped an operator on capable hardware (tens of
// GB of RAM) from using more of their own resources. The cap's only remaining job is to
// turn a fat-fingered value (OCP_TUI_POOL_SIZE=99999999) into a bounded number rather
// than a fork bomb; 32 is generous for real use and still does that.
export const POOL_MAX_SIZE = 32;
...
// pool rather than guessing — an unparseable size must never silently boot a pile of processes.
