...
export function validateJsonSchema(value, schema, path = "$", strict = false, root = schema) {
  const errors = [];
  ...
  if (typeof schema.$ref === "string") {
    ...
    return validateJsonSchema(value, { ...resolved, ...siblings }, path, strict, root);
  }

  // Composition. allOf: every branch must pass. anyOf: at least one. oneOf: exactly one.
  if (Array.isArray(schema.allOf)) {
    for (const sub of schema.allOf) errors.push(...validateJsonSchema(value, sub, path, strict, root));
  }
  if (Array.isArray(schema.anyOf)) {
    if (!schema.anyOf.some(sub => validateJsonSchema(value, sub, path, strict, root).length === 0)) {
      errors.push(`${path}: does not match any of the allowed schemas (anyOf)`);
    }
  }
  if (Array.isArray(schema.oneOf)) {
    const matches = schema.oneOf.filter(sub => validateJsonSchema(value, sub, path, strict, root).length === 0).length;
