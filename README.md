...
| `scripts/lib/start-sh.mjs` | Builds the standalone `start.sh` launcher `setup.mjs` writes (issue #246) — extracted so this logic can be driven by injected fake binaries in tests, since `setup.mjs` itself must never be executed by the test suite. Its macOS port check uses absolute-path `lsof`/`netstat` (a restricted `PATH`, e.g. a launchd job's default environment, can omit `/usr/sbin` entirely) plus a `netstat` cross-check for `lsof`'s privilege-gap ambiguity; the Linux port check is a simpler bare `ss -tlnp` and gets neither the absolute-path treatment nor the cross-check. |
