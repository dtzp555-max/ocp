...
- **Every request returns 500 with `error: unknown option '--system-prompt-file'`** — your Claude CLI predates that flag. OCP passes the system prompt as a file rather than on the command line (see [Security](#security)); the flag is measured working on `claude` **2.1.233+** and is absent from `claude --help`'s option list, so you cannot confirm it there. Run `claude update` (or reinstall the CLI) and restart OCP. There is **no version gate** for the Claude CLI today — the proxy starts fine and only fails per request ([#455](https://github.com/dtzp555-max/ocp/issues/455)).
